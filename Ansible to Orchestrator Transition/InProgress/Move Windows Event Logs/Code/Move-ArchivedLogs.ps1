...
foreach ($server in $servers) {
    ...
    try {
        ...
        # The work moves to the DESTINATION here, so the stage has to move with it BEFORE
        # anything touches it. The Test-Path below is the first thing to reach the share,
        # and if the stage is still the source one, a destination failure gets reported
        # against the source path -- which reads as "the source share denied us" and sends
        # the reader to the wrong machine entirely.
