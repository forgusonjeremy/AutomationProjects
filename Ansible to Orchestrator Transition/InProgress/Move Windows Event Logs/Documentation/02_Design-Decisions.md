# Design Decisions — Move Archived Logs

Why this automation looks the way it does. Read this first if you knew the Ansible
playbooks.

> **Scope.** This document covers **Move Archived Logs** only. Its housekeeping partner,
> *Remove Old Archived Logs*, is a separate package with its own design document. Where a
> decision here was made jointly with that one, it says so.

---

## 1. Four playbooks became one workflow

The source was four playbooks that all did the same job:
...
The move logic inside all four is the **same `Move-files` function**, copied four times.
What actually differed was two choices, made independently.

### Choice one: how servers are picked, and the answer is an Active Directory group
...
### Choice two: how files are reached, and the answer is UNC from one host
...
hand — unworkable past a handful of machines, and pointless when one host can reach them
all over the administrative share.

So: **one AD group in, one PowerShell host doing the work, files pulled over UNC.** That
is the `file-move_with-UNCPath_AD-Group` shape, and the other three collapse into it.

---

## 2. The Active Directory lookup left PowerShell

This is the biggest change, and it is what the rest of the design hangs off. It is also
the part that is **unique to this automation** — its partner never touches AD at all.
...
### Disabled computer accounts are skipped

A disabled account is a decommissioned server. Attempting it produces a timeout and an
error for something nobody expected to work. It is skipped **by name in the log**, so a
server missing from the results can be accounted for without guessing.
...
**Remove Old Archived Logs** uses the same one.*
...
> **This bug was specific to the move playbooks.** The cleanup function shipped alongside
> them, `Remove-OldFiles-UNCPath`, used `AddDays(-$OlderThanDays)` and was correct. Two
> functions in the same toolbox, disagreeing about the sign of the same parameter.
...
*Shared decision, applied to both automations.*

Both now start with **report only** switched on. A run that is not explicitly told to make
changes will only ever list what it would have done.

> **Report only proves less than it looks.** It never writes to the destination — both the
> folder check and the per-file check sit behind the report-only test — so a clean
> report-only run says nothing about whether the destination share is writable. Only a
> live run proves that. This is why Part 4 of the Testing Plan exists as a separate part
> rather than as a footnote to Part 3.
...
## 6. Paths are names, never IP addresses

*Shared decision. It applies to both automations, and it bites both in the same way.*

Every path this automation uses must be a hostname or FQDN.

The source paths look after themselves — they are built from AD computer names, so they
are always FQDNs. The **destination** is the one an operator types, and it is the one that
gets written as an IP while testing and then left that way.

That does not work, and it fails misleadingly. Kerberos authenticates to a service
principal name, which is built from a host name; there is no SPN for an IP literal, so a
UNC path written with one cannot use Kerberos at all. It falls back to NTLM and is refused
with `Access is denied` — which reads as a share-permissions fault on a share whose
permissions are perfectly fine.

The confusing part is the asymmetry: the source paths keep working, because they are
FQDNs, so the failure looks like the *servers* are refusing rather than the destination.

> **Why Ansible never hit this.** The playbooks did not rely on delegation at all. They
> used `become_method: runas` with an explicit password, which performs a fresh logon
> holding real credentials, and that logon can authenticate onward by itself. A script
> that worked under Ansible can therefore fail here **unchanged**, in an environment where
> nothing else has changed.

---

## 7. Errors name the path they happened on

`Move-ArchivedLogs.ps1` tracks which operation it is performing and reports it alongside
any failure:
...
Windows says `Access is denied` and nothing else. On its own that leaves a reader unable
to tell the source share from the destination share — and which one it was is the first
thing anyone needs to know, because they are different machines with different owners.

This matters more than it sounds. An earlier version advanced that tracking too late, so a
**destination** failure was reported against the **source** path. The message named a
server that was working correctly, and the investigation went to the wrong machine
entirely.
...
| Deleting the archived files afterwards | That is the other automation's job, deliberately. Moving and deleting are separate decisions with separate retention periods, and combining them would mean one mistake could destroy logs that were never archived. |
