...
| **Automated regression checks** | **447** offline checks across the two most complex deliverables — no AD, SMTP, PowerShell host or vRO appliance required to run them |

## What was delivered, by project

| # | Project | Was | Is |
|---|---|---|---|
| 1 | **Server Reboot Report** | 2 Ansible report playbooks | 1 read-only workflow |
| 2 | **Admin Accounts (PKI) Report** | 3 job templates, 2 of them on a forked script | 1 workflow, one shared script |
| 3 | **Service Account Expiration Report** | 1 job template | 1 workflow *(in progress)* |
...
|---|---|---|
| 1 | Server Reboot Report | Simplest; establishes the common pattern everything else reuses |
| 2 | Admin Accounts Report | Same pattern, richer output — the report-quality story |
| 3 | Service Account Expiration | Strongest defect story; visibly the same product as #2 |
...
## 3. Service Account Expiration Report  *(in progress)*
...
This is the finale and the most visual demo of the session — run it live end to end.
...
## The pattern behind the findings *(closes Part A — the transition work)*

Across the seven transition deliverables, the defects found in the existing automation share
**one failure mode**. This is the strongest single message available:
