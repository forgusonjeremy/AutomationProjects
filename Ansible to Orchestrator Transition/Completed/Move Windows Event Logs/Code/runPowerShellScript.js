...
// Which of the two ran is logged. They behave differently in a way that only shows up
// one hop away, so a run that fails against \\<server>\C$ has to be able to say which
// call it used -- otherwise the log looks identical either way and the question can only
// be settled by remembering when an action was last pasted, which is no way to settle it.
var invocation;
...
if (typeof psHost.invokeScript === "function") {
    ...
    System.log("Invoking via psHost.openSession() -- a separate session, as this plug-in " +
               "does not expose invokeScript() on the host");
