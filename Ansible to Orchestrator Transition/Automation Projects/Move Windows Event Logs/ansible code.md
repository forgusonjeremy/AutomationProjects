...
***Playbook***
---
- name: Move archive log files off to the share location
  hosts: all
  gather_facts: no
  ...
  - name: "Scipt Operations"
    ...
    - name: Run PowerShell script
      ...
      ansible.windows.win_command: powershell.exe -ExecutionPolicy ByPass
        ...
        -FileShareTarget "{{var_FileShareTarget}}"
  ...
  tasks:
    - name: Get computer members
      ansible.windows.win_shell: |
        ...
        $secPassword = ConvertTo-SecureString "{{ WinRM_password }}" -AsPlainText -Force
        $cred = New-Object System.Management.Automation.PSCredential("{{ WinRM_username }}", $secPassword)
        
        try {
            $group = Get-ADGroup -Identity "{{ group_dn }}" -Properties Members -Credential $cred -ErrorAction Stop
            
            $computers = @()
            foreach ($member in $group.Members) {
                try {
                    $comp = Get-ADComputer -Identity $member -Properties DNSHostName -Credential $cred -ErrorAction Stop
    ...
    - name: Check for errors in AD query
      fail:
        msg: "AD query failed: {{ (ad_query.stdout | from_json).Error }}"
      when:
        - ad_query.stdout is defined
        - ad_query.stdout | trim | length > 0
        - (ad_query.stdout | from_json).Error is defined
    ...
    - name: Set empty list if no results
      set_fact:
        ad_computers: []
      when: ad_computers is not defined
...
group_dn: myServers
source_path: C$\Windows\System32\winevt\Logs
target_path: \\fileserver.mydomain.local\mdcarchivelog$\Windows
file_filter: Archive-*.evtx
days_old: '0'
  ...
  tasks:
    - name: Get computer members
      ansible.windows.win_shell: |
        ...
        try {
            $group = Get-ADGroup -Identity "{{ group_dn }}" -Properties Members -Server "{{ DomainName }}" -ErrorAction Stop
            
            $computers = @()
            foreach ($member in $group.Members) {
                try {
                    $comp = Get-ADComputer -Identity $member -Properties DNSHostName -Server "{{ DomainName }}" -ErrorAction Stop
                    $computers += [PSCustomObject]@{
                        Name = $comp.Name
                        DNSHostName = $comp.DNSHostName
                    }
                } catch {
                    # Silent - this member is not a computer
                }
            }
            
            # Always output valid JSON (even if empty array)
            if ($computers.Count -gt 0) {
                $computers | ConvertTo-Json -Compress
            } else {
                '[]'
            ...
            # Output error as JSON so we can parse it
            @{Error = $_.Exception.Message} | ConvertTo-Json
        }
      register: ad_query
      failed_when: false  # Don't fail the playbook, handle errors in next task
...
***playbook***
---
- name: "Retrieve Computer Objects from {{domain_name}}"
  hosts: all
  gather_facts: false
  run_once: true
  ...
  tasks:
    - name: Get computer members
      ansible.windows.win_shell: |
        ...
        $computers = $group.Members | ForEach-Object {
            try {
                Get-ADComputer -Identity $_ -Properties DNSHostName, OperatingSystem, DistinguishedName
            } catch {
            }
        }
...
- name: Executing function against the dynamic group
