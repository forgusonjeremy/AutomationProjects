...
$AgeCutoff = (Get-Date).AddDays(-$MinimumAgeDays)
[array]$allResults = @()

Write-Log "=== $script:ReportName start (Action=$Action) ==="
Write-Log "Filters: MinimumDiskGB=$MinimumDiskGB, MinimumAgeDays=$MinimumAgeDays (cutoff $($AgeCutoff.ToString('yyyy-MM-dd')))"
        ...
        foreach ($ds in $datastores) {
            ...
            try {
                ...
                foreach ($grp in $orphanGroups) {
                    ...
                    if ($lastModified -and $lastModified -gt $AgeCutoff) { continue }
                    $ageDays = if ($lastModified) { [int]((Get-Date) - $lastModified).TotalDays } else { $null }
...
$tblVcenterSummary = Convert-RowsToCompactHtml -Rows $summaryByVcenter -Accent '#1f2937' -EmptyText 'No orphaned VMDKs matched the configured size and age filters.'
$tblOrphaned = Convert-RowsToStyledHtml -Rows $sorted -Accent '#b91c1c' -EmptyText 'No orphaned VMDKs matched the configured size and age filters.'
  ...
  <tr><td align="center" style="padding:8px 6px;">
    <table role="presentation" width="960" cellpadding="0" cellspacing="0" style="max-width:960px;width:100%;background:#ffffff;border:1px solid #e5e7eb;border-radius:8px;overflow:hidden;">
      ...
      <tr><td style="padding:8px 18px 2px;font-family:Segoe UI,Arial,sans-serif;color:#374151;font-size:12px;line-height:1.35;">
        Compares datastore VMDK files to registered VM layout inventory. Lists unattached VMDK groups at least <b>$MinimumDiskGB GB</b> and not modified in the last <b>$MinimumAgeDays days</b> (on or before $($AgeCutoff.ToString('yyyy-MM-dd'))). Full detail attached as CSV. Review before deleting.
      </td></tr>
...
$csvFile = Join-Path $DebugDir ('orphaned_vmdks_{0}.csv' -f (Get-Date -Format 'yyyyMMdd_HHmmss'))
...
if ($eMailReport -eq 'yes') {
    ...
    if (Test-Path $csvFile) { $mailParams.Attachments = $csvFile }
    Send-MailMessage @mailParams
    Write-Log "Email sent to $MailToString (attachment: $(Split-Path $csvFile -Leaf))"
}
