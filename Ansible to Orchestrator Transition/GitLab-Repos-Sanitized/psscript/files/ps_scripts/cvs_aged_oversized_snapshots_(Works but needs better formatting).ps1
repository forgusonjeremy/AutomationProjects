...
$critical = @($allResults | Where-Object { $_.ConsolidationNeeded -eq $true -or $_.SnapshotDeltaSizeGB -ge $OversizedSnapshotGB } | Sort-Object @{Expression='SnapshotDeltaSizeGB';Descending=$true}, @{Expression='SnapshotAgeDays';Descending=$true})
$agedOnly = @($allResults | Where-Object { $_.ConsolidationNeeded -ne $true -and $_.SnapshotDeltaSizeGB -lt $OversizedSnapshotGB } | Sort-Object @{Expression='SnapshotDeltaSizeGB';Descending=$true}, @{Expression='SnapshotAgeDays';Descending=$true})
...
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#f3f4f6;">
  <tr><td align="center" style="padding:24px 12px;">
    <table role="presentation" width="1120" cellpadding="0" cellspacing="0" style="max-width:1120px;width:100%;background:#ffffff;border:1px solid #e5e7eb;border-radius:8px;overflow:hidden;">
      <tr><td style="background:#1f2937;padding:20px 28px;font-family:Segoe UI,Arial,sans-serif;color:#ffffff;font-size:19px;font-weight:700;">Aged / Oversized Snapshot Report</td></tr>
      ...
      <tr><td style="padding:8px 28px 4px;">
        <div style="font-family:Segoe UI,Arial,sans-serif;font-size:13px;font-weight:700;color:#b91c1c;margin:14px 0 0;padding-bottom:6px;border-bottom:2px solid #b91c1c;">&#9679; Highest risk &nbsp;&ndash;&nbsp; oversized and/or consolidation needed</div>
        $tblCritical
        <div style="font-family:Segoe UI,Arial,sans-serif;font-size:13px;font-weight:700;color:#c2410c;margin:14px 0 0;padding-bottom:6px;border-bottom:2px solid #c2410c;">&#9679; Aged snapshots &nbsp;&ndash;&nbsp; older than $SnapshotAgeDays days, sorted biggest first</div>
        $tblAged
        <div style="font-family:Segoe UI,Arial,sans-serif;font-size:13px;font-weight:700;color:#7c2d12;margin:14px 0 0;padding-bottom:6px;border-bottom:2px solid #7c2d12;">&#9679; Consolidation-needed VMs without matching snapshot rows</div>
        $tblConsol
      </td></tr>
...
$allResults | Sort-Object @{Expression='SnapshotDeltaSizeGB';Descending=$true}, @{Expression='SnapshotAgeDays';Descending=$true} | Export-Csv -NoTypeInformation -Path $csvFile -Encoding utf8
