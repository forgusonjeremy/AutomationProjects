[CmdletBinding()]
param (
    [ValidateSet('move-archived-logs-ByCN','Delete-OldFiles-UNC-Share','tls-fix','move-archived-logs','clean-ServerDisk','Invoke-ServerReboot','Get-ServerPendingRebootStatus','Get-ServerRebootReportStatus-ByCN','Get-AllAdmin-Accounts','Get-ServiceAccountExpiration','Get-ServiceAccountExpiration-ByGroup','get_datastores_75_100_used','VMware_Disable_SSH')]
    [string]$Action,
    ...
    [string]$SMTPServer,
    [Parameter(Mandatory=$false, ValueFromPipeline=$true)]
    [string]$MailToString = 'user3@dom3.example',
    [Parameter(Mandatory=$false, ValueFromPipeline=$true)]
    [string]$MailCcString = 'user3@dom3.example',
    [Parameter(Mandatory=$false, ValueFromPipeline=$true)]
...
function Get-ListOfUsers-ByGroup {
    [CmdletBinding()]
    param(
        [parameter(Mandatory = $true)]
    ...
    process {
        $userProps = 'SamAccountName','UserPrincipalName','smartcardlogonrequired','DisplayName',`
                     'Office','Enabled','Lockedout','pwdLastSet','AccountExpirationDate','WhenCreated','Description'

        if([string]::IsNullOrEmpty($DomainName)){
            $OUQuery1 = Get-ADGroupMember -Identity "$($SecurityGroup)" -Recursive |
                Where-Object { $_.objectClass -eq 'user' } |
                Get-ADUser -Properties $userProps
        }else{
            $OUQuery1 = Get-ADGroupMember -Identity "$($SecurityGroup)" -Server $DomainName -Recursive |
                Where-Object { $_.objectClass -eq 'user' } |
                Get-ADUser -Server $DomainName -Properties $userProps
        }
        return $OUQuery1
    }
}
...
# Matches the newer Power(1) card-style report format while keeping report logic
# and data collection unchanged. Inline styles + bgcolor attributes are used
# because Outlook/Exchange often strips or ignores <style> blocks.
...
function New-CvsReportHtml {
    param(
        ...
        [string]$Summary,
        [string]$Sections,
        [string]$FooterScriptName = 'svc con.ps1'
    )
...
Function GenerateReport($data) { # GenerateReport
    $rawTable = $data | ConvertTo-Html -Fragment
    $rawTable = $rawTable -replace 'False','<font color="red">False</font>'
    $rawTable = $rawTable -replace 'smartcardlogonrequired', 'SmartCardEnabled'

    $tblReport = Format-CvsTableHtml -Fragment ($rawTable -join "`n") -Accent '#1f2937'
    $sections = New-CvsReportSectionHtml -Title 'Report Details' -TableHtml $tblReport -Accent '#1f2937'
    [string]$body = New-CvsReportHtml -Title 'VMT Report' `
        -Summary "Generated report for $($Global:UserName)." `
        -Sections $sections `
        -FooterScriptName 'svc con.ps1'
...
function GenerateReportServerPendingRebootStatus($data){
    ...
    $HeaderNote = "The list of servers were base on the security group called <b>$($HeaderNotesSubstr)</b>. The script performed remote WMI registry call to the server to determine if reboot is required. The RPC server service on the remote server need to be available and accessible by the script or it will failed."
    ...
    $rawTable = $data | Sort-Object -Property ComputerName, PendingReboot, ComputerlastBootUptime, CrashOnAuditFail  | ConvertTo-Html -Property ComputerName, PendingReboot, ComputerlastBootUptime, UserSession, CrashOnAuditFail -Fragment
    $rawTable = $rawTable -replace 'True','<font color="orange">Required Reboot</font>'
    $rawTable = $rawTable -replace 'ComputerlastBootUptime', 'lastBootUptime'
    $rawTable = $rawTable -replace 'Error Accessing Server','<font color="red">Error Accessing Server</font>'
    $rawTable = $rawTable -replace 'False','<font color="green">No Action required</font>'
    $rawTable = $rawTable -replace 'YES','<font color="red">YES</font>'

    $tblReboot = Format-CvsTableHtml -Fragment ($rawTable -join "`n") -Accent '#c2410c' -CenterCells
    $sections = New-CvsReportSectionHtml -Title 'Pending Reboot Status' -TableHtml $tblReboot -Accent '#c2410c'
    [string]$body = New-CvsReportHtml -Title 'Server Pending Reboot Report' `
        -Summary $HeaderNote `
        -Sections $sections `
        -FooterScriptName 'svc con.ps1'
...
function GenerateReportPKI($data){
    ...
    $rawTable = $data | Sort-Object -Property smartcardlogonrequired,SamAccountName  | ConvertTo-Html -Property SamAccountName, UserPrincipalName, smartcardlogonrequired, displayName, whenCreated, description -Fragment
    $rawTable = $rawTable -replace 'False','<font color="red">False</font>'
    $rawTable = $rawTable -replace 'smartcardlogonrequired', 'SmartCardEnabled'
    $rawTable = $rawTable -replace 'whenCreated', 'CreatedOn'
    $rawTable = $rawTable -replace 'displayName', 'DisplayName'
    $rawTable = $rawTable -replace 'description', 'Description'

    $tblPKI = Format-CvsTableHtml -Fragment ($rawTable -join "`n") -Accent '#1f2937'
    $sections = New-CvsReportSectionHtml -Title 'Smartcard / PKI Account Status' -TableHtml $tblPKI -Accent '#1f2937'
    [string]$body = New-CvsReportHtml -Title 'Admin Account PKI Report' `
        -Summary "Accounts are sorted by smartcard compliance status and account name." `
        -Sections $sections `
        -FooterScriptName 'svc con.ps1'
...
function GenerateReportServiceAccountExpiration($data){
    ...
    $rawTable = $data | ConvertTo-Html -Property SamAccountName, DisplayName, Office, Enabled, Lockedout, "PW Age", "PW LastSet",Description, AccountExpirationDate -Fragment
    #$body = $body -replace 'Thieu N','<font color="red">Thieu N</font>'

    #$body = $body -replace 'SamAccountName', 'AccountName'
    $rawTable = $rawTable -replace 'AccountExpirationDate', 'ExpirationDate'

    $tblServiceAccounts = Format-CvsTableHtml -Fragment ($rawTable -join "`n") -Accent '#1f2937'
    $sections = New-CvsReportSectionHtml -Title 'Service Account Password Age / Status' -TableHtml $tblServiceAccounts -Accent '#1f2937'
    [string]$body = New-CvsReportHtml -Title 'Service Account Expiration Report' `
        -Summary "Service accounts are listed with password age, last set date, account status, and description." `
        -Sections $sections `
        -FooterScriptName 'svc con.ps1'
...
function Main($Action){
    ...
    switch($Action){
        ...
        'Get-ServiceAccountExpiration-ByGroup'{
            $strModule = 'ActiveDirectory'
            if (Invoke-Module $strModule){
                $Result = Get-ListOfUsers-ByGroup -SecurityGroup $ADGroupMember -DomainName $DomainName |
                    Select-Object "SamAccountName","DisplayName","Office","pwdLastSet","AccountExpirationDate","WhenCreated","Enabled","Lockedout","Description" |
                    Select-Object -Property "SamAccountName","DisplayName","Office","Enabled","Lockedout",`
                    @{Name="PW Age";Expression={if($_.pwdLastSet -ne 0){(new-TimeSpan([datetime]::FromFileTimeUTC($_.PwdLastSet)) $($Global:Today)).days}else{0}}},"AccountExpirationDate",`
                    @{Name="PW LastSet";Expression={[datetime]::FromFileTime($_."pwdLastSet")}},"Description","WhenCreated" | Sort-Object -Property "PW Age" -Descending
        ...
        'get_datastores_75_100_used'{
            ...
            $tblHigh = Format-CvsTableHtml -Fragment ($ds_high -join "`n") -Accent '#b91c1c'
            $tblMed  = Format-CvsTableHtml -Fragment ($ds_med  -join "`n") -Accent '#c2410c'
            $tblLow  = Format-CvsTableHtml -Fragment ($ds_low  -join "`n") -Accent '#a16207'

            $sections = ''
            $sections += New-CvsReportSectionHtml -Title "Critical &nbsp;&ndash;&nbsp; &ge; $high% used" -TableHtml $tblHigh -Accent '#b91c1c'
            $sections += New-CvsReportSectionHtml -Title "Warning &nbsp;&ndash;&nbsp; $med% to $high% used" -TableHtml $tblMed -Accent '#c2410c'
            $sections += New-CvsReportSectionHtml -Title "Elevated &nbsp;&ndash;&nbsp; $low% to $med% used" -TableHtml $tblLow -Accent '#a16207'

            [string]$body = New-CvsReportHtml -Title 'Datastore Capacity Report' `
                -Summary "Datastores between <b>$low%</b> and <b>100%</b> used <i>and</i> with less free space than uncommitted (thin-provision) space are listed below, grouped by severity." `
                -Sections $sections `
                -FooterScriptName 'svc con.ps1' 
        ...
        'VMware_Disable_SSH'{
            ...
            ## Email Body
            $rawTable = $enabled | Sort-Object vCenter,Host | ConvertTo-Html -Fragment
            $tblEnabled = Format-CvsTableHtml -Fragment ($rawTable -join "`n") -Accent '#1f2937'
            $sections = New-CvsReportSectionHtml -Title 'Hosts remediated by automation' -TableHtml $tblEnabled -Accent '#1f2937'

            [string]$body = New-CvsReportHtml -Title 'VMware SSH Enabled Report' `
                -Summary "<b>$($enabled.Count)</b> hosts were set to Enabled and were Disabled by the automated script." `
                -Sections $sections `
                -FooterScriptName 'svc con.ps1'
