[CmdletBinding()]
param (
    [ValidateSet('tls-fix','move-archived-logs','clean-ServerDisk','Invoke-ServerReboot','Get-ServerPendingRebootStatus','Get-ServerPendingRebootStatusDP','Get-AllAdmin-Accounts','Get-ServiceAccountExpiration','get_datastores_75_100_used','VMware_Disable_SSH')]
    [string]$Action,
...
if ($DomainOUsFile){
    ...
    $DomainOUs = Get-Content -Raw -Path $DomainOUsFile

    if ($DomainOUs) {
        $DomainOUsMap = ConvertFrom-Json $DomainOUs
    } else {
        $DomainOUsMap = @{}
    }
...
Function GenerateReportPKI-v2($data, $DomainOUsMap){

    $Style = "<style>BODY{background-color:white;font-family:Segoe UI;font-size:9px}TABLE{border-width: 1px;border-style: solid;border-color: black;border-collapse: collapse;}TH{border-width: 1px;padding: 2px;border-style: solid;border-color: black;background-color:gray;color:white}TD{border-width: 1px;padding: 2px;border-style: solid;border-color: black;background-color:lightgrey}</style>"
    [string]$body = $data | Sort-Object -Property smartcardlogonrequired,SamAccountName  | ConvertTo-Html -Property SamAccountName, UserPrincipalName, smartcardlogonrequired, displayName, whenCreated, description -Head $Style
    $body = $body -replace 'False','<font color="red">False</font>'
    $body = $body -replace 'smartcardlogonrequired', 'SmartCardEnabled'
    $body = $body -replace 'whenCreated', 'CreatedOn'
    ...
    # Add OU footnote
    $ouFootnote = "<hr><b>The accounts were pulled from the following OUs Queried:</b><ul>"
    foreach ($domain in $DomainOUsMap.PSObject.Properties.Name) {
        $ouFootnote += "<li><b>$domain</b><ul>"
...
function Get-ListOfUsers-MultiDomain {
    ...
    foreach ($domain in $DomainOUsMap.PSObject.Properties.Name) {
        ...
        foreach ($ou in $OUs) {
            Write-Host "DEBUG: Querying domain: $domain, OU: $ou, SC: $SC"
            if ($SC -eq $true -or $SC -eq $false) {
            ...
            Write-Host "DEBUG: Found $($OUQuery1.Count) users in $ou"
            $results += $OUQuery1
        }
    }
    return $results
...
function Main($Action){
    ...
    switch($Action){
        ...
        'Get-AllAdmin-Accounts'{
            $strModule = 'ActiveDirectory' 
            if (Invoke-Module $strModule){
                $Result = Get-ListOfUsers-MultiDomain -DomainOUsMap $DomainOUsMap -SC $true | select SamAccountName, UserPrincipalName, smartcardlogonrequired, displayName, whenCreated, description, Enabled
                $Global:PKIEnabledCount = ($Result | Measure-Object).Count
                $Result2 = Get-ListOfUsers-MultiDomain -DomainOUsMap $DomainOUsMap -SC $false | select SamAccountName, UserPrincipalName, smartcardlogonrequired, displayName, whenCreated, description, Enabled
                $Global:PKIDisabledCount = ($Result2 | Measure-Object).Count
                ...
                if ($Result.Count -eq 0) {
                    Write-Host "DEBUG: No users found for the given OUs and domains."
                }
                GenerateReportPKI-v2 $Result $DomainOUsMap
            }else{ 
                Write-Log "Error: Unable to import PS Modules $($strModule) or it is NOT install" $true 
            }
        }
