[CmdletBinding()]
param (
    [ValidateSet('Invoke-ServerReboot','Get-ServerPendingRebootStatus','Get-ServerPendingRebootStatusDP','Get-AllAdmin-Accounts','Get-ServiceAccountExpiration','get_datastores_75_100_used','VMware_Disable_SSH')]
    [string]$Action,
...
Function Invoke-BatchFile { # Invoke-BatchFile -BatchFile "C:\TempDir\get-vm.ps1" 
    [CmdletBinding()]
    Param(
        [Parameter(Mandatory=$true)]
        [ValidateNotNullOrEmpty()]
    ...
    Process{
        Try {
            ...
            if (Test-Path $BatchFile) {
                ...
                Write-Log "Error: File not found." $true
            }
...
Function Remove-files { # Remove-files -Path "C:\TempDir" -FilterOn "*.ps1" -Days '-30'
    ...
    Process{
        Try { 
            if( $Force -eq 'true'){
                Get-ChildItem -Path $Path -Recurse -File -Filter $FilterOn | Where-Object { $_.LastWriteTime -lt $dateTime } | Remove-Item -Force
            }else{
                Get-ChildItem -Path $Path -Recurse -File -Filter $FilterOn | Where-Object { $_.LastWriteTime -lt $dateTime } | Remove-Item
            }
...
function Get-ListOfUsers{
    ...
    process{
        if($SC -eq $true -OR $SC -eq $false){
            ...
            $OUQuery1 = Get-ADUser -Filter {SmartcardLogonRequired -eq $SC}  `
...
function Invoke-ServerReboot{
    ...
    process{
        ...
        Try {
            ...
            Write-log "Info: invoke server rebooting $($ServerName)" $true
...
function GenerateReportServerPendingRebootStatus($data){
    ...
    $data | ConvertTo-Html -Property ComputerName, PendingReboot, ComputerlastBootUptime -Head $Header | Out-File -FilePath Report.html 
    [string]$body = $data | Sort-Object -Property ComputerName, PendingReboot, ComputerlastBootUptime  | ConvertTo-Html -Property ComputerName, PendingReboot, ComputerlastBootUptime, UserSession -Head $Style
    $body = $body -replace 'True','<font color="red">Required Reboot</font>'
    ...
    switch($Action){
        'Get-Users-SCenable'{
            ...
            if (Invoke-Module $strModule){
                Get-ListOfUsers -SC $true | select SamAccountName, UserPrincipalName, smartcardlogonrequired
            }else{ 
        ...
        'Get-AllAdmin-Accounts'{
            ...
            if (Invoke-Module $strModule){
                ...
                $Result = Get-ListOfUsers -SC $true | select SamAccountName, UserPrincipalName, smartcardlogonrequired, displayName, whenCreated, description, Enabled
                foreach($r in $Result){
                ...
                $Result2 = Get-ListOfUsers -SC $false | select SamAccountName, UserPrincipalName, smartcardlogonrequired, displayName, whenCreated, description, Enabled
                if($Result2 -eq $null){
                    $Global:PKIDisabledCount = 0
        ...
        'Get-ServiceAccountExpiration'{
            $strModule = 'ActiveDirectory' 
            if (Invoke-Module $strModule){
                $Result = Get-ListOfUsers | Select-object "SamAccountName","DisplayName","Office", "pwdLastSet","AccountExpirationDate","WhenCreated","Enabled","Lockedout","Description" |
        ...
        'Set-L3-Admin-Accounts'{
            $strModule = 'ActiveDirectory' 
            if (Invoke-Module $strModule){
                $OUQuery1 = Get-ListOfUsers -SC $false
                $objT = @()
                $obj = @()
        ...
        'Get-ServerPendingRebootStatus'{
            ...
            if (Invoke-Module $strModule ){
                ...
                $ListOfComputers = Get-ListOfServers -SecurityGroup $ADGroupMember # | select -ExpandProperty Name
                $ListOfServers = @()
                ...
                # Write-Log "Info: number of server required reboot # $($NumberOfRequiredReboot)"
        ...
        'Invoke-ServerReboot'{
            ...
            if (Invoke-Module $strModule ){
                ...
                $ListOfComputers = Get-ListOfServers -SecurityGroup $ADGroupMember
                $ListOfServers = @()
                foreach($L in $ListOfComputers){ $ListOfServers += ($L.name) }
                $Result = Get-RebootStatus -ComputerNames $ListOfServers
                $NumberOfRequiredReboot = 0
