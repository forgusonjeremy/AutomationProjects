...
VERSION=
CRON_JOB=1
CRONMD=">> ~/mdatp_cron_job.log"
CRONMDEUbuntu="0 6 * * sun [ $(date +%d) -le 15 ] && sudo apt-get install --only-upgrade mdatp $CRONMD"
CRONMDERedhat="0 6 * * sun [ $(date +%d) -le 15 ] &&  sudo yum update mdatp -y $CRONMD"
CRONMDERSLES="0 6 * * sun [ $(date +%d) -le 15 ] && sudo zypper update mdatp $CRONMD"
...
verify_connectivity()
{
    ...
    if which wget; then
        connect_command="wget -O - --quiet --no-verbose --timeout 10 https://cdn.x.cp.wd.microsoft.com/ping --no-check-certificate"
    elif which curl; then
        connect_command="curl --silent --connect-timeout 10 --insecure https://cdn.x.cp.wd.microsoft.com/ping"
    else
...
remove_repo()
{
    # TODO: add support fedora
    if [ $DISTRO == 'sles' ] || [ "$DISTRO" = "sle-hpc" ]; then
    echo "Removing $DISTRO Microsoft-$CHANNEL.list" 
    	    run_quietly "$PKG_MGR_INVOKER removerepo packages-microsoft-com-$CHANNEL" "failed to remove repo"
    elif [ "$DISTRO_FAMILY" = "debian" ]; then 
    echo "Removing $DISTRO Microsoft-$CHANNEL.list"
    	    run_quietly "rm /etc/apt/sources.list.d/microsoft-$CHANNEL.list"  "failed to remove repo"           
...
scale_version_id()
{
    ...
    if [ "$DISTRO_FAMILY" == "fedora" ]; then
        if [[ $VERSION == 7* ]] || [ "$DISTRO" == "amzn" ]; then
            ...
            SCALED_VERSION=8
        elif [[ $VERSION == 9* ]] || [ "$DISTRO" == "fedora" ]; then
            SCALED_VERSION=9
        else
...
set_cron_job()
{
    # echo "[v] backing up crontab"
    crontab -l > "$(date +"%FT%H%M").mde-cron-backup.txt"

    # echo "[v] setting agent upgrade cron job"    
    if [ "$DISTRO_FAMILY" = "debian" ]; then
         ( crontab -l | grep -v -F "$CRONMD" ; echo "$CRONMDEUbuntu") | crontab -
    elif [ "$DISTRO_FAMILY" = "fedora" ]; then
        ( crontab -l | grep -v -F "$CRONMD" ; echo "$CRONMDERedhat" ) | crontab -
    elif [ "$DISTRO_FAMILY" = "sles" ]; then
        ( crontab -l | grep -v -F "$CRONMD" ; echo "$CRONMDESLES" ) | crontab -
    else    
        script_exit "not able to set agent upgrade cron job" $ERR_CRONJOB_FAILED
    fi

    log_info "[v] set agent upgrade cron job"
}
...
if [[ -z "${INSTALL_MODE}" && -z "${ONBOARDING_SCRIPT}" && -z "${PASSIVE_MODE}" && -z "{$CRON_JOB}" && ${#tags[@]} -eq 0 ]]; then
